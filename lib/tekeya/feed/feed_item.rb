  ...
  module Feed
    class FeedItem
      ...
      def self.from_redis(key, act_actor = nil)
        ...
        act_id          = key_components[0]
        act_type        = key_components[3]
        act_time        = Time.at(key_components[4].to_i)
        
        if act_actor.nil?
          actor_class = key_components[1].safe_constantize
          act_actor = actor_class.where(:"#{actor_class.entity_primary_key}" => key_components[2]).first
        end
